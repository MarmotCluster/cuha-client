      ...
      if (i.isAdminTool) {
        return (
          ...
          >
            ...
            <button type="button" className="inner-category-item__toolbtn" style={{ backgroundColor: '#d0342c', color: '#f4f4f4' }}>
              삭제
            </button>
          </div>
        );
  ...
  useEffect(() => {
    if (type) {
      switch (type) {
        case 'all':
          forums
            .get(`/posts`, { params: { page: 0, size: 9 } })
            .then((res) => {
          ...
          forums.get(`/comments`, { params: { page: 0, size: 5 } }).then((res) => {
            // console.log('받아온 5개의 댓글');
            setRequests((state) => ({ ...state, comments: res.data }));
          });
          forums.get(`/members`, { params: { page: 0, size: 5 } }).then((res) => {
            // console.log('받아온 5개의 계정');
            setRequests((state) => ({ ...state, accounts: res.data }));
          });
          break;
        case 'board':
          forums.get(`/posts`, { params: { page: 0, size: 10 } }).then((res) => {
            setRequests((state) => ({ ...state, posts: res.data }));
          });
          break;
        case 'comment':
          forums.get(`/comments`, { params: { page: 0, size: 10 } }).then((res) => {
            setRequests((state) => ({ ...state, comments: res.data }));
          });
          break;
        case 'account':
          forums.get(`/members`, { params: { page: 0, size: 10 } }).then((res) => {
            setRequests((state) => ({ ...state, accounts: res.data }));
          });
          break;
        default:
