...
import { createAccount, logoutAccount, testAxios, updateAccountWithoutPassword } from '../actions';
...
const EditUser = (props) => {
  ...
  useEffect(() => {
    forums.get('/members/info').then((res) => {
      ...
      const { department, email, gender, name, phoneNumber, profileFilename, studentId, username } = res.data;
      ...
      case 'department':
        return (
          <React.Fragment key={key}>
            ...
            <div className="menu-container__section-form__error" style={{ height: `${err['department'] * 2}rem` }}>
              <p className={seto.language === 1 ? '' : 'en'}>입력란이 비었습니다.</p>
            </div>
          </React.Fragment>
        );
      ...
      >
        <div className="menu-container__section-form-file">
          <div
            ...
            style={{
              backgroundImage: newImage.url ? `url(${newImage.url})` : `url(${forums.defaults.baseURL}/profiles/${currentUser.profileUrl})`,
            }}
          >
            <input
        ...
        <div className="menu-container">
          ...
          <section className="menu-container__section" style={{ width: 'calc(100% - 10rem)', maxWidth: '48rem', padding: '2rem' }}>
            <form name="login" onSubmit={(e) => funcOnSubmit(e)}>
              ...
              <button className="menu-container__section-form-button-outer" type="button" onClick={() => setIsShownAlert((state) => true)}>
                계정 탈퇴를 원합니다.
              </button>
